...
print(
    f"DEBUG main.py: about to call build_effects() with " f"display={type(display).__name__}",
    flush=True,
)
effects = build_effects(_boot_settings, display=display)
print(
    f"DEBUG main.py: build_effects() returned {len(effects)} effects: " f"{[type(e).__name__ for e in effects]}",
    flush=True,
)
...
print(
    f"DEBUG main.py: about to call coordinator.start(); " f"coordinator.is_bound()={coordinator.is_bound()}",
    flush=True,
)
coordinator.start()
...
# is self-throttled to DEFAULT_TICK_INTERVAL_S (now 5s, unified with
# the MQTT publish cadence — see Decision 1 in openspec/changes/
# add-sign-status-reports/design.md). StatusSnapshot (the dataclass)
# lives in status.py alongside the writer; this closure sources the
# live values from the manager + thread state at write time.
