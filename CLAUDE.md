...
- `lib_shared/selector.py` — `MessageSelector` (issue #26). The Pi and the browser preview both call `pick(messages, now, event_log, current_event_type, favorites)` — identical class, identical algorithm. Reads `display_recency` from the event log, normalizes `send_recency` over the eligible set, applies an additive `W_DISPLAY * disp + W_SEND * send + W_FAVORITE * (1 if fav else 0)` score, and breaks ties on `(-score, sent_at, message.id)`. Behavioral knobs (`W_DISPLAY`, `W_SEND`, `W_FAVORITE`, `SATURATION_SECONDS`, `OFFSET_SECONDS`, `USE_WEIGHTED_SELECTOR`) are module-level constants — NOT in `settings.toml`. Ships dark with `USE_WEIGHTED_SELECTOR=False`.
