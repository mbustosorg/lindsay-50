# pi-power-commands Specification
...
The Pi's `force-upgrade` handler MUST exec the loader at the resolved target version (read at boot from `GET /api/sign/settings`) and let the loader perform its standard SHA-check + stage + probe + swap. If already at target, the loader is a no-op (idempotent).
...
- **THEN** browser confirms via a `confirm()` dialog, Flask publishes exactly one `force-upgrade` envelope, returns 202, the Pi's existing `MessageManager.dispatch` routes the action to `command_handlers.force_upgrade`, which `os.execvpe`s into the loader; the loader reads the resolved target via `GET /api/sign/settings` on its next boot phase and either completes the upgrade or falls through to running the current version (existing self-healing path)
...
- **THEN** the Pi halts its display loop, systemd restarts the loader, the loader reads the resolved target via `GET /api/sign/settings` on boot (alongside `AUTO_UPDATE` from `settings.toml`) and decides whether to upgrade; the dashboard Live pill transitions through `live → unsure → offline → live` as the Pi goes down and comes back
...
`main.py` MUST register these handlers **alongside** the existing `check-for-update` registration. The handlers MUST NOT take responsibility for routing or auth — `MessageManager.dispatch` handles that. The handlers MUST log their invocation (INFO level).

The existing `check-for-update` registration is RETAINED in this change (kept on the Pi alongside the three new ones during the transitional period). It will be removed in a follow-up change once all deployed Pis are confirmed to be running the new `GET /api/sign/settings` boot-fetch path.

#### Scenario: Pi boots and receives all three new action types via MQTT

- **WHEN** Flask publishes `action=force-upgrade`, then `action=restart`, then `action=shutdown`, on the same `MQTT_TOPIC`
- **THEN** each envelope is dispatched to its registered handler in order; the handler logs and executes its side effect; the `check-for-update` registration is still present (post-change, kept for transitional compatibility)
...
- Set env `LINDSAY50_ACTIVE_SHA` to current active SHA if known (short form).
