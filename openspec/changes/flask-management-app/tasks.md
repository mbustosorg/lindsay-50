...
- [x] 1.1 Create `Procfile` with `web: litestream restore -if-replica-exists /app/db.sqlite && litestream replicate -exec "flask run --host=0.0.0.0"`
- [x] 1.2 Create `litestream.yml` with R2 bucket configuration
- [x] 1.3 Create `heart-sms-receiver/requirements.txt` with flask, adafruit-io, litestream, jinja2
- [x] 1.4 Create Cloudflare R2 bucket and configure credentials
...
- [x] 2.4 Create `lib/filters.py` with `apply(message, config)` and `display_list(messages, config)`, using Python `re` module

## 3. Flask App Refactor

- [x] 3.1 Refactor `heart-sms-receiver/main.py` to import from `lib/storage`
- [x] 3.2 Update Twilio webhook to use `storage.put_message()` before returning TwiML
...
- [x] 3.5 Add `PUT /api/config` accepting full config JSON, storing and publishing
...
- [x] 4.6 Create `GET /preview` showing `filters.display_list()` output
- [x] 4.7 Wire settings/filters/message actions to trigger config publish

## 5. Config Publish (Interface Only)

- [x] 5.1 Stub publish function in `lib/` (publish to MQTT topic OR make HTTP POST to ESP32 endpoint — pending communication decision)
- [x] 5.2 Call stub publish function after any config change via admin UI
