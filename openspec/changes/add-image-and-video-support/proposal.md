...
- Detect `NumMedia` > 0 from `request.form`. For each `MediaContentType{i}` / `MediaUrl{i}` pair, download the bytes (authenticated via Twilio Basic Auth — the API supports the `MediaUrl` directly without the Twilio webhook signature) and write to S3 under `images/{YYYY-MM}/...` (for `image/*`) or `videos/{YYYY-MM}/...` (for `video/*`). GIF is `image/gif` and lands in `images/`.
- Compute the S3 key, then build the wire tuple as `{"type": <mime>, "url": "<s3-key>"}` (the s3-key is the relative path under the bucket, not a pre-signed URL — see *Image fetch path* below).
