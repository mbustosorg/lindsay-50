...
- New `GET /api/sign/boot-config` endpoint on Flask returns `{"expected_sha": "<sha>"}` (renamed from the v1 `/api/sign/expected-sha`; the response shape is just the SHA and nothing else). Auth via existing `X-API-Key`.
