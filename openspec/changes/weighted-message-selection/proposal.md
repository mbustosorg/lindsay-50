...
- Add a Pi-local append-only event log on disk (`heart-matrix-controller/data/events.jsonl` by default, path configurable). Each render event is one JSONL line: `{event_type, message_id, timestamp, sent_at, favorite}`. The renderer writes the event immediately after each message advances.
...
- Add a `favorite` flag per message (boolean) so the selector can tilt the score toward senders' favorites without mutating the message body. Where `favorite` lives in storage is an implementer choice; the selector reads it from the message record.
...
- `heart-matrix-controller/settings.toml` — add `EVENT_LOG_PATH`, `SELECTOR_W_DISPLAY`, `SELECTOR_W_SEND`, `SELECTOR_W_FAVORITE`, `SELECTOR_SATURATION_SECONDS`, `SELECTOR_OFFSET_SECONDS`, and `USE_WEIGHTED_SELECTOR` keys.
