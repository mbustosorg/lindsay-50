...
echo ""
echo "Adafruit IO local server (Docker, Node 10 for util.isDate compatibility):"
echo "  # Build image:"
echo "  docker build -t adafruit-io-local -f Dockerfile.adafruit-io ."
echo "  # Run:"
echo "  docker run -d --name aio-local -p 8082:8082 \\"
echo "    -e AIO_SERVER_PORT=8082 \\"
echo "    -e AIO_SERVER_KEY=test-key \\"
echo "    -e AIO_USERNAME=test-user \\"
echo "    adafruit-io-local"
echo "  # Create a feed (one-time):"
echo "  curl -s -X POST http://localhost:8082/api/feeds \\"
echo "    -H 'Content-Type: application/json' -H 'X-AIO-Key: test-key' \\"
echo "    -d '{\"name\":\"test-feed\",\"key\":\"test-feed\"}'"
echo "  # Then set in settings.toml:"
echo "  AIO_BASE_URL = 'http://localhost:8082'"
echo "  AIO_USERNAME = 'test-user'"
echo "  AIO_KEY = 'test-key'"
echo "  AIO_FEED = 'test-feed'"
echo "  NOTE: The Python adafruit-io client uses the v2 API path"
echo "        (/api/v2/{user}/feeds/{feed}/data) which the local server"
echo "        does not implement. AIO publish will fail locally but works"
echo "        with real Adafruit IO credentials. S3/MinIO will work fully."
echo ""
