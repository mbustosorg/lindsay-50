...
echo "  # And run: aws configure (region: us-east-1, key: minioadmin/minioadmin)"
echo ""
echo "Adafruit IO local server (for local AIO testing):"
echo "  npm install -g forever adafruit-io  # Note: requires Node <12 for full compatibility"
echo "  # Polyfill wrapper (Node 12+ compat, needed because adafruit-io uses deprecated util.isDate):"
echo "  cat > /tmp/aio-server-wrapper.js << 'WRAPPER'"
echo "    const util = require('util');"
echo "    util.isDate = d => d instanceof Date;"
echo "    util.isRegExp = r => r instanceof RegExp;"
echo "    require(process.argv[2]);"
echo "  WRAPPER"
echo "  # Configure (one-time):"
echo "  adafruit-io server config --username test-user --key test-key"
echo "  # Start:"
echo "  AIO_SERVER_PORT=8082 node /tmp/aio-server-wrapper.js \\"
echo "    \$(npm root -g)/adafruit-io/server/index.js"
echo "  # Then set in settings.toml:"
